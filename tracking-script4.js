(function () {
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script4'));
  ...
  function sendToAllPlatforms(eventName, data = {}) {
    ...
    if (typeof gtag === 'function' && CONFIG.googleAdsId) {
      ...
      console.log(`[Tracking] gtag not ready or Google Ads ID missing: ${eventName}`);
    }
    ...
    if (typeof gtag === 'function' && CONFIG.ga4MeasurementId) {
      ...
      console.log(`[Tracking] GA4 measurement ID missing or gtag not ready: ${eventName}`);
    }
